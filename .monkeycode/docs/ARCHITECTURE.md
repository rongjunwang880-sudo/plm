...
2. 火山大模型接收用户问题、命中资料和资料初判，输出 TRIZ 方案。
3. 火山阶段响应为空或失败时，DeepSeek 生成接替方案。
4. DeepSeek 对 TRIZ 方案输出质询结论、证据缺口、风险、边界条件和待验证问题。
5. 智谱清言基于方案和质询输出最终工程总结。
6. 页面在请求期间显示四段斜角进度条，并将开始按钮切换为禁用的运行状态。
